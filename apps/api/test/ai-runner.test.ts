...
import {
  ...
  LocalAgentRunner,
} from "../src/modules/ai/runner.js";

function createRepository(messages: Message[] = []) {
  return {
...
describe("LocalAgentRunner attachments", () => {
  it("loads a saved conversation as soon as a thread connects", async () => {
    ...
    const runner = new LocalAgentRunner(repository);
  ...
  it("replaces the agent's pre-seeded binary messages before running", async () => {
    ...
    expect(repository.persistMessages).toHaveBeenCalledWith("thread-id", [
      expect.objectContaining({
        content: [
          expect.objectContaining({
            type: "document",
...
describe("LocalAgentRunner display stream", () => {
