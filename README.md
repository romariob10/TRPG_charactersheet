...
Ассистент листа персонажа умеет искать справочные фрагменты о правилах НРИ и
отвечать со ссылками на источники. Начальная база содержит материалы на русском
и английском для D&D 5e (2014), Pathfinder 2e Remaster, Fate Core и Call of
Cthulhu 7e. Например: «Как работает преимущество в D&D 5e 2014?».

Это небольшой справочник, а не полные книги правил. Редакция указывается явно;
если нужного правила нет, ассистент должен сообщить об этом. Поиск работает
локально и не требует отдельного API-ключа или сервиса эмбеддингов. Ответы
генерирует уже настроенный AI-провайдер.

Устройство поиска, ограничения и добавление материалов описаны в
[документации базы знаний](docs/RPG_KNOWLEDGE.md).
...
Состояние локальных веток, объединение историй и исправления CI/CD описаны в
[аудите веток](docs/BRANCH_AUDIT.md).
