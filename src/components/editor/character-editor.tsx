...
export function CharacterEditor({
  ...
  const persistField = useCallback(
    async (fieldId: string): Promise<boolean> => {
      ...
      try {
        ...
        const result = (await response.json()) as FieldMutationResponse & {
          error?: string;
        };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`character:${initialCharacter.id}`, {
        config: { presence: { key: initialCharacter.currentUserId } },
      })
      .on("presence", { event: "sync" }, () =>
        setOnlineUsers(Object.keys(channel.presenceState()).length || 1),
      )
      .on(
        "postgres_changes",
        {
          ...
          filter: `character_id=eq.${initialCharacter.id}`,
        },
        (payload: { new: Record<string, unknown> }) => {
          ...
          if (!next.field_id) return;
          setFields((current) => {
            const updated = current.map((field) => {
              if (
                field.id !== next.field_id ||
                (next.version ?? 0) <= field.version
              )
                return field;
              if (
                ...
                if (next.updated_by !== initialCharacter.currentUserId)
                  setNotice(t("remoteUpdate"));
                return { ...field, version: next.version ?? field.version };
              }
              return {
                ...field,
                value: next.value ?? null,
                version: next.version ?? field.version,
  ...
  }, [initialCharacter.currentUserId, initialCharacter.id, t]);
  ...
  async function createInvite() {
    ...
    if (!response.ok || !result.url)
      return setNotice(result.error ?? "Could not create invite");
